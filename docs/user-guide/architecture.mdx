...
- **HTTP proxy:** The BE proxies requests via `/ws/:serviceCode/*`. A
  ...
  ```bash
  # From the FE (or any JWT-authenticated client) through the BE proxy:
  curl -H "Authorization: Bearer <jwt>" \
    http://localhost:3001/ws/emailsender/api/v1/entities/providers/list
  # → { "rows": [...], "total": 0, "page": 1, "page_size": 20 }

  # Direct to the microservice (bypassing the BE, for debugging):
  curl http://localhost:3003/api/v1/entities/providers/list
  ```
